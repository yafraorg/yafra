...
# argument:		no args
#
# author:		yafraorg@gmail.com
#
# tag:			$Name:  $
# revision:		$Revision: 2.2 $
# scm info:     $Id: mapo-dbsetup.pl,v 2.2 2008-07-27 17:01:47 mwn Exp $
#-------------------------------------------------------------------------------
...
use vars qw($TKmenu_bar);
use vars qw($TKfile_mb);
use vars qw($TKtdb_mb);
use vars qw($TKhelp_mb);
...
$TKtop->title("yafraDB database setup");
$TKcolor = "green";
...
$TKbody_frm->Label(-text => "yafraDB database setup wizard")->pack(-side => 'top');
...
$mp_dbdelfile = $TKtop->getOpenFile(-defaultextension => ".*",
		-initialdir => Cwd::cwd(), -initialfile => $mp_genfile, -title => "yafraDB deletion script");
}
sub MP_DBgenfile
{
$mp_dbgenfile = $TKtop->getOpenFile(-defaultextension => ".*",
		-initialdir => Cwd::cwd(), -initialfile => $mp_genfile, -title => "yafraDB generate batch");
}
sub MP_DBdumpfile
{
$mp_dbdumpfile = $TKtop->getOpenFile(-defaultextension => ".*",
		-initialdir => Cwd::cwd(), -initialfile => $mp_genfile, -title => "yafraDB generate with dump batch");
}
...
sub MP_INITdo
{
	if (not defined $TKdbinit)
		...
		my $dbdata_p = $TKnb_init->add("dbdata", -label => "yafraDB data", -underline => 0);
...
sub MP_USERconf
{
	if (not defined $TKuser)
		{
		$TKuser = $TKtop->DialogBox(-title => "yafraDB user configurations:", -buttons => ["OK", "Cancel"]);
...
sub MP_DATAprint
{
	...
	$TKprint->Label(-text => "yafraDB user:     $mp_user")->pack(-side => 'top', -anchor => 'nw');
	$TKprint->Label(-text => "yafraDB user level: $mp_userlevel")->pack(-side => 'top', -anchor => 'nw');
	...
	mpdebug("yafraDB user: $mp_user");
	mpdebug("yafraDB user Lvl: $mp_userlevel");
...
sub MP_HELPshow
{
	if (not defined $TKhelp)
		...
		$TKhelp->Label(-text => "This is the wizard to setup/create your yafra database.")->pack(-side => 'top');
...
sub MP_DBdosetup
{
	...
	if ($status == 2)
		...
		if (not defined $TKdropdialog)
			{
			$TKdropdialog = $TKtop->Dialog(-text => 'Drop current yafraDB ?',
				-bitmap => 'question', -title => 'Delete yafraDB', -default_button => 'NO',
				-buttons => [qw/YES NO/]);
			}
