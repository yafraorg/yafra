...
# scm info:     $Id: mapodb.pm,v 2.2 2005-03-11 19:17:26 mwn Exp $
#----------------------------------------------------------
...
my $dbhost = "";
my $os = "";
my	$logfile = "yafra-dbsetupsql.log";
my $dbversion = 3;
use vars qw($dbh);
...
sub MP_DBcheckroot
{
	...
	db_debug ("  No ROOT schema present, starting to create yafraDB now");
	$sth->finish;
	$dbh->commit;
	# disconnect as SYSTEM user from the DB
	$dbh->disconnect;
...
sub MP_DBusers
{
	...
	if ($db eq "mssql")
		...
		{
		# create user
		$dbh->do("create user $mpusr identified by $mpusrpw profile");
		if ($DBI::err)
			{
			db_debug("error inserting new yafra user: $DBI::errstr");
			return(1);
			}
