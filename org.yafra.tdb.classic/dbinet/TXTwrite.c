/*D***********************************************************
 * Modul:     MPNET - database insert  of TEXT object                          
 *                                                            
...
static char rcsid[]="$Header: /yafra/cvsroot/mapo/source/dbinet/TXTwrite.c,v 1.2 2008-11-02 19:55:51 mwn Exp $";

int TXTinsert(char *buffer)
{
	...
	for ( i=0; i < anzahl; i++ )
		...
		for ( oldptr=ptr=buffer, anzchar=(int)0; ;ptr++ )
			...
			if ( *ptr == '\n'  || *ptr == '\t' || *ptr == NULL )
				...
				status = MPAPIinsertEntry((int)_TEXTE, (char *)&txt, (BEZEICHNUNG *)NULL);
